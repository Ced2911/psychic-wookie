...
static int process_video_header_mdec(AVFormatContext *s)
{
    ...
    { AVRational tmp__0 = {1,15}; ea->time_base = tmp__0; }
...
static int process_video_header_cmv(AVFormatContext *s)
{
    ...
    fps = avio_rl16(s->pb);
    if (fps)
        { AVRational tmp__1 = {1, fps}; ea->time_base = tmp__1; }
        ...
        switch (blockid) {
            ...
            case kVGT_TAG:
                ea->video_codec = AV_CODEC_ID_TGV;
                 { AVRational tmp__2 = {1, 15}; ea->time_base = tmp__2; }
                break;
...
static int ea_read_header(AVFormatContext *s)
{
    ...
    if (ea->video_codec) {
        ...
        st->r_frame_rate =
#endif
        { AVRational tmp__3 = {ea->time_base.den, ea->time_base.num}; st->avg_frame_rate = tmp__3; }
    }
...
AVInputFormat ff_ea_demuxer = {
    "ea",
    NULL_IF_CONFIG_SMALL("Electronic Arts Multimedia"),
